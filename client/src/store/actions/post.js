...
import { apiGetPosts,apiGetPostsLimit,apiGetPostsLimit2,apiGetNewPosts,apiGetPostsLimitAdmin } from "../../services/post";
...
export const getPostLimit2 = (query)=> async (dispatch) => {
    try {
        const response = await apiGetPostsLimit2(query);
        // console.log(response)
        if(response?.data.err === 0){
            dispatch({ 
                type : actionTypes.GET_POST_LIMIT,
